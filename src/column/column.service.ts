...
export class ColumnService {
    ...
    async findOne(id: number) {
        return await this.columnRepository.findOne({where: {id: id}, relations: {author_: true, cards: true}});
    }

    async create(column: CreateColumnDto, user: UserEntity) {
        return await this.columnRepository.save({title: column.title, author_: user});
    }
